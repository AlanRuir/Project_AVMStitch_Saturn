...
VideoEncoderH26XCPU::VideoEncoderH26XCPU(uint32_t cols, uint32_t rows, CodecType codec)
...
VideoEncoderH26XCPU::~VideoEncoderH26XCPU() noexcept
{
    if (codec_context_)
    {
...
bool VideoEncoderH26XCPU::Encode(uint8_t* frame, uint32_t frame_size, uint64_t timestamp)
{
